...
function weightedMetrics(arr){const paid=sum(arr,'paid');const nav=sum(arr,'nav');return {nav,paid,dist:sum(arr,'dist'),unfunded:sum(arr,'unfunded'),commitment:sum(arr,'commitment_revised'),dpi:paid?sum(arr,'dist')/paid:null,rvpi:paid?nav/paid:null,tvpi:paid?sum(arr,'total_value')/paid:null}}
...
const HTML_SLIDES = {
  ...
  8: {render: renderMarketSentimentSlide, width: 1920, height: 1080, afterRender: renderMarketSentimentSlideAfterRender},
  9: {render: renderStrategyDeepDiveSlide, width: 1920, height: 1080, afterRender: renderStrategyDeepDiveSlideAfterRender},
  10: {render: renderLiquidityOptionsDividerSlide, width: 1920, height: 1080},
  11: {render: renderDiscussionPointsSlide, width: 1920, height: 1080, afterRender: animateDiscussionPointsSlide},
  12: {render: renderNextStepsSlide, width: 1920, height: 1080, afterRender: animateNextStepsSlide},
  13: {render: renderBasisOfPreparationSlide, width: 1920, height: 1080, afterRender: animateBasisOfPreparationSlide},
  14: {render: renderAnnexesDividerSlide, width: 1920, height: 1080},
  15: {render: renderPortfolioHoldingsSlide, width: 1920, height: 1080, afterRender: bindPortfolioHoldingsSort},
  16: {render: renderTangibleContactSlide, width: 1920, height: 1080},
  17: {render: renderDisclaimerSlide, width: 1920, height: 1080, afterRender: animateDisclaimerSlide}
};
...
function openModal(title,sub,body){hideTableExport();$('#modalTitle').textContent=title;$('#modalSub').textContent=sub||'';$('#modalBody').innerHTML=body;$('#modal').classList.add('open');$('#modal').setAttribute('aria-hidden','false');syncOptionsMenuVisibility()}
function closeModal(){hideTableExport();$('#modal').classList.remove('open');$('#modal').setAttribute('aria-hidden','true');syncOptionsMenuVisibility()}
...
function holdingsCols(){return [
  ...
  {key:'unfunded_pct',label:t('table.col.unfunded'),type:'num',align:'num',render:d=>fmtPct(d.unfunded_pct)},
...
function holdingsTotalCells(cols,visible){
  ...
  return cols.map(c=>{
    ...
    else if(c.key==='unfunded_pct')v=m.commitment?fmtPct(m.unfunded/m.commitment):'—';
...
function openDataLens(title,sub,arr,opts={}){arr=arr.slice();const m=weightedMetrics(arr);const mgrCount=new Set(arr.map(d=>d.manager)).size;const single=!!opts.singleManager;const mgrKpi=single?'':`<div class="lensKpi"><div class="v">${mgrCount}</div><div class="l">${t('kpi.managers')}</div></div>`;const sortNone=`<option value="">${t('lens.sortDefault')}</option>`;const sortOpts=sortNone+(single?`<option value="nav">${t('lens.sortNav')}</option><option value="name">${t('lens.sortFundAz')}</option><option value="vintage">${t('lens.sortVintage')}</option><option value="dpi">${t('lens.sortDpi')}</option>`:`<option value="nav">${t('lens.sortNav')}</option><option value="name">${t('lens.sortManagerAz')}</option><option value="count">${t('lens.sortMostFunds')}</option><option value="dpi">${t('lens.sortDpi')}</option>`);openDrawer(title,sub||t('lens.includedMgrs',{n:arr.length,mgrs:mgrCount}),`${opts.intro?`<div class="lensIntro">${opts.intro}</div>`:''}<div class="lensKpis${single?' lensKpis-5':''}"><div class="lensKpi"><div class="v">${fmtMoney(m.nav)}</div><div class="l">${t('kpi.nav')}</div></div><div class="lensKpi"><div class="v">${arr.length}</div><div class="l">${t('kpi.interests')}</div></div>${mgrKpi}<div class="lensKpi"><div class="v">${fmtX(m.dpi)}</div><div class="l">${t('kpi.dpi')}</div></div><div class="lensKpi"><div class="v">${fmtX(m.tvpi)}</div><div class="l">${t('kpi.tvpi')}</div></div><div class="lensKpi"><div class="v">${m.commitment?fmtPct(m.unfunded/m.commitment):'—'}</div><div class="l">${t('kpi.unfunded')}</div></div></div><div class="lensTools"><input id="lensSearch" placeholder="${single?t('lens.filterFunds'):t('lens.filterFundsOrManagers')}"><select id="lensSort">${sortOpts}</select><button id="lensOpenTable">${t('lens.openTable')}</button></div><div id="lensList"></div>`);const input=$('#lensSearch'),sort=$('#lensSort'),list=$('#lensList'),openBtn=$('#lensOpenTable');function render(){const q=(input.value||'').trim().toLowerCase();let visible=arr.filter(d=>!q||[d.investment,d.manager,d.strategy,String(d.vintage)].some(v=>String(v||'').toLowerCase().includes(q)));if(!visible.length){list.innerHTML=`<div class="emptyLens">${t('lens.empty')}</div>`;return}const k=sort.value;if(single){if(k)visible.sort((a,b)=>k==='name'?String(a.investment).localeCompare(String(b.investment),I18N.locale):k==='vintage'?(Number(b.vintage)||0)-(Number(a.vintage)||0):k==='dpi'?(b.dpi||-99)-(a.dpi||-99):(b.nav||0)-(a.nav||0));const maxFund=Math.max(...visible.map(d=>d.nav||0),1);list.innerHTML=`<div class="lensFundsFlat">${visible.map(d=>lensFundRow(d,maxFund)).join('')}</div>`}else{let groups=managerGroups(visible);if(k)groups.sort((a,b)=>k==='name'?a.manager.localeCompare(b.manager,I18N.locale):k==='count'?b.items.length-a.items.length:k==='dpi'?(b.dpi||-99)-(a.dpi||-99):b.nav-a.nav);list.innerHTML=groups.map((g,gi)=>{const maxFund=Math.max(...g.items.map(d=>d.nav||0),1);const funds=(k==='nav'?g.items.slice().sort((a,b)=>(b.nav||0)-(a.nav||0)):g.items).map(d=>lensFundRow(d,maxFund)).join('');return `<div class="lensGroup ${gi===0?'open':''}"><div class="lensManager"><div class="lensArrow">›</div><div><div class="lensName">${esc(g.manager)}</div><div class="lensMeta">${t('lens.groupMeta',{count:t('common.interestCount',{n:g.items.length}),strategy:esc(g.strategy),dpi:fmtX(g.dpi),tvpi:fmtX(g.tvpi)})}</div></div><div class="lensValue">${fmtMoney(g.nav)}</div></div><div class="lensFunds">${funds}</div></div>`}).join('');$$('.lensManager',list).forEach(el=>el.addEventListener('click',()=>el.closest('.lensGroup').classList.toggle('open')))}$$('.fundRow',list).forEach(el=>el.addEventListener('click',ev=>{ev.stopPropagation();fundDetail(el.dataset.idx)}))}input.addEventListener('input',render);sort.addEventListener('change',render);openBtn.addEventListener('click',()=>openHoldings(d=>arr.includes(d),t('table.holdingsSuffix',{title})));render()}
