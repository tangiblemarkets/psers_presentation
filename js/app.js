...
function openManager(name,sourceRows){const arr=(sourceRows||CFG.rows).filter(d=>d.manager===name);const dom=primaryStrategy(arr);openDataLens(name,t('lens.managerSub',{count:t('common.interestCount',{n:arr.length}),strategy:dom}),arr,{intro:t('lens.introManager'),singleManager:true});}
